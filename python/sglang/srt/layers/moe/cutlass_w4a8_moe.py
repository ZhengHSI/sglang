...
from sglang.srt.layers.moe.triton_w4a8_int8_moe_mm import triton_w4a8_int8_moe_mm
...
def triton_w4aint8_moe(
    ...
    - w2_q (torch.Tensor): The second set of int4-quantized expert weights.
        ...
        (the weights are passed transposed and int4-packed)
    - w1_scale (torch.Tensor): The fp32 scale to dequantize w1_q.
        Shape: [num_experts, K // 128, N * 8]
    - w2_scale (torch.Tensor): The fp32 scale to dequantize w2_q.
        Shape: [num_experts, N // 128, K * 4]
    - topk_weights (torch.Tensor): The weights of each token->expert mapping.
    - a_strides1 (torch.Tensor): The input strides of the first grouped gemm.
    - b_strides1 (torch.Tensor): The weights strides of the first grouped gemm.
    - c_strides1 (torch.Tensor): The output strides of the first grouped gemm.
    ...
    # print(w2_q.shape)
    # print(w2_scale.shape)

    # # Validate scale shapes for Triton format
    # assert (
    #     w1_scale.shape[1] == w1_q.shape[2] * 2 // 128
    # ), "W1 scale shape mismatch"
    # assert (
    #     w2_scale.shape[1] == w2_q.shape[2] * 2 // 128
    # ), "W2 scale shape mismatch"
    ...
    # gateup_input = torch.empty(
    #     a.shape, dtype=torch.int8, device=device
    # )
    ...
    c1 = torch.empty((m * topk, n * 2), device=device, dtype=torch.half)
    c2 = torch.zeros((m * topk, k), device=device, dtype=torch.half)
    # c1 = torch.empty((m, n * 2), device=device, dtype=torch.half)
