...
// NEW: INT8 版本的 kernel
template <typename ElementA, typename ElementB, typename ElementC, typename ElementAccumulator>
__global__ void int4_int8_get_group_gemm_starts(
    int32_t* expert_offsets,
    ElementA** a_offsets,
    ElementB** b_offsets,
    ElementC** out_offsets,
...
// NEW: INT8 版本的宏
#define __CALL_W4A8_INT8_GET_STARTS_KERNEL(TENSOR_C_TYPE, C_TYPE)                         \
  else if (out_tensors.dtype() == TENSOR_C_TYPE) {                                        \
    int4_int8_get_group_gemm_starts<cutlass::int8_t, cutlass::int8_t, C_TYPE, float>      \
        <<<1, num_experts, 0, stream>>>(                                                  \
            static_cast<int32_t*>(expert_offsets.data_ptr()),                             \
            static_cast<cutlass::int8_t**>(a_ptrs.data_ptr()),                            \
            static_cast<cutlass::int8_t**>(b_ptrs.data_ptr()),                            \
            static_cast<C_TYPE**>(out_ptrs.data_ptr()),                                   \
            static_cast<float**>(a_scales_ptrs.data_ptr()),                               \
            static_cast<cutlass::bfloat16_t**>(b_scales_ptrs.data_ptr()),                 \
            static_cast<cutlass::int8_t*>(a_tensors.data_ptr()),                          \
            static_cast<cutlass::int8_t*>(b_tensors.data_ptr()),                          \
            static_cast<C_TYPE*>(out_tensors.data_ptr()),                                 \
            static_cast<float*>(a_scales.data_ptr()),                                     \
            static_cast<cutlass::bfloat16_t*>(b_scales.data_ptr()),                       \
...
// NEW: INT8 版本的函数
void run_int4_int8_get_group_gemm_starts(
    torch::Tensor const& expert_offsets,
    torch::Tensor& a_ptrs,
    torch::Tensor& b_ptrs,
    torch::Tensor& out_ptrs,
    ...
    torch::Tensor const& a_scales,
    torch::Tensor const& b_scales) {
  TORCH_CHECK(a_tensors.dtype() == torch::kInt8);      // 改为 INT8
  TORCH_CHECK(b_tensors.dtype() == torch::kInt8);
  TORCH_CHECK(a_scales.dtype() == torch::kFloat32);
  TORCH_CHECK(b_scales.dtype() == torch::kBFloat16);
