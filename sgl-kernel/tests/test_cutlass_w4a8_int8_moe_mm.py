import pytest
import torch
from sgl_kernel import cutlass_w4a8_int8_moe_mm
...
def test_int4_int8_grouped_gemm_single_expert(batch_size):
    # Test parameters
    num_experts = 1
    ...
    # Quantize input
    a_q = torch.clamp(torch.round(a / a_scale), -128.0, 127.0).to(torch.int8)
    ...
    # Compare results
    try:
        torch.testing.assert_close(c, c_ref, rtol=0.01, atol=0.1)
...
def test_int4_int8_grouped_gemm_multi_experts(batch_size, k, n, num_experts):
    torch.manual_seed(0)
    dtype = torch.bfloat16
    device = "cuda"
    debug = False
    ...
    a_q_perm = torch.clamp(torch.round(a_perm / a_scale), -128.0, 127.0).to(torch.int8)
    ...
    c = torch.empty_like(c_perm, dtype=dtype)
...
#         ref_w = (w[i].to(torch.float32) * ref_w_scale_repeat).to(dtype)
#         c_tok = torch.matmul(a_tok.to(dtype), ref_w.t().to(dtype)) * a_scale
#         c_tok = c_tok.to(dtype)
#         c_ref[token_idx] = c_tok.to(dtype)

#     return c_ref

def ref_grouped_gemm(c, a, a_scale, w, w_scale, num_experts, experts_selection_result):
    dtype = torch.bfloat16
    c_ref = torch.zeros_like(c.to(dtype))
    for i in range(num_experts):
        token_idx = torch.where(experts_selection_result == i)[0]
        if len(token_idx) == 0:
            continue
        ...
        ref_w_scale_repeat = w_scale[i].repeat_interleave(128, dim=1).to(torch.float32)
        ref_w = (w[i].to(torch.float32) * ref_w_scale_repeat).to(dtype)
        c_tok = torch.matmul(a_tok, ref_w.t().to(dtype))
        c_tok = c_tok.to(dtype)
        c_ref[token_idx] = c_tok.to(dtype)
